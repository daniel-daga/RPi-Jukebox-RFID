...
ExecStart=${LIBRESPOT_BIN} --name "${DEVICE_NAME}" --backend pulseaudio --bitrate 160 --initial-volume 100 --volume-ctrl fixed
Restart=on-failure
RestartSec=5
...
echo ""
echo "One-time activation: open the Spotify app on your phone (same WiFi,"
echo "same Spotify account), tap the device picker and select '${DEVICE_NAME}'."
echo "After that, RFID cards can start playback on it directly."
echo ""
